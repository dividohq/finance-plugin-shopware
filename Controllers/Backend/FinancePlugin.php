    ...
    extends Shopware_Controllers_Backend_ExtJs
{
    ...
    public function updateFinanceAction()
    {
        ...
        if (!(array_key_exists($orderStatus, $cancelStatuses)
            || $orderStatus == $activateStatus)
        ) {
        ...
        Helper::log(
            'Update Webhook response received: '.json_encode($updateResponse),
            'info'
        );
    ...
    public function activateOrderAction()
    {
        ...
        $activateResponse = $activateService::activateApplication(
            $order[0]['transactionID'],
            $order_amount,
            $items
        );
        ...
        if ($activateResponse->error == true) {
            $this->View()->assign(
                [
                    'success' => false,
                    'message' => "Could not activate order: "
    ...
    public function refundOrderAction()
    {
        ...
        $refundResponse = $refundService::refundApplication(
            $order[0]['transactionID'],
            $order_amount,
            $items,
            $order[0]['orderNumber']
    ...
    public function cancelOrderAction()
    {
        ...
        $cancelResponse = $cancelService::cancelApplication(
            $order[0]['transactionID'],
            $order_amount,
            $items,
            $order[0]['orderNumber']
    ...
    /**
     * Function to retrieve all plans in the database
     *
     * @return void
     */
